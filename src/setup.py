...
cap.set(cv2.CAP_PROP_POS_FRAMES, 1110)
trackers = []
frame_count = 0
student_amount = 4


f = open('calibration/lesson1/face_locations.csv', 'w')
writer = csv.writer(f)

while True:
    # Grab a single frame of video
    ret, frame = cap.read()    # Convert the image from BGR color (which OpenCV uses) to RGB
    frame = cv2.resize(frame, (640, 360))  # Resize video
    ...
    if frame_count % 30 == 0:
        ...
        print(face_count)
        if len(face_locations) == student_amount:
